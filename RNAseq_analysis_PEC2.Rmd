...
Ejemplo del listado de genes, ordenado por p-valor ajustado (decreciente), incluyendo el símbolo del gen y su nombre:

```{r genes with symbol ordered by p-value}
res_SFIvsNIT_trimmed <- res_SFIvsNIT[, c(7, 9, 6, 2)] # Select columns
...
knitr::kable(head(res_SFIvsNIT_trimmed[order(res_SFIvsNIT_trimmed$padj), ]),
             ...
             align = 'clcc',
             digits = c(4, 2),
             caption = 'Ejemplo de genes con expresión diferencial entre los grupos SFI y NIT, ordenados por menor p-valor ajustado. Incluye el símbolo y el nombre del gen correspondientes a la base de datos Entrez. NA = casos en los que no hay correspondencia entre el código ENSEMBL y la base de datos Entrez.')
