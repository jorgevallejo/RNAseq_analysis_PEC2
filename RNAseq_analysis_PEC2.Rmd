...
# Remove samples that we are not going to use any more
# rm(list = c("targets_ELI", "targets_NIT", "targets_SFI"))
...
```{r order counts_subset rows by Group and sample name, eval= FALSE}
# Esto lo dejo sin evaluar porque creo que no es necesario
...
targets_sampled <- targets_sampled[order(targets_sampled$ShortName), ]
```
...
```{r inspect sample relationships}
# Estimate dispersions
if (file.exists("intermediateData/ddsMatB.RData")){
  load("intermediateData/ddsMatB.RData")
}else{
  ddsMatB <- estimateDispersions(ddsMat)
  save(ddsMatB, file = "intermediateData/ddsMatB.RData")
}
...
if (file.exists("intermediateData/vsd.RData")){
  ...
  vsd <- varianceStabilizingTransformation(ddsMatB)
  save(vsd, file = "intermediateData/vsd.RData")
}
```

```{r inspect PCA plot, fig.cap= 'Análisis de componentes principales. El 70% de la variabilidad parece explicado por el grupo al que pertenece la muestra, ya que vemos una clara separación en el eje horizontal del grupo ELI frente a los otros dos grupos (SFI y NIT), que aparecen entremezclados.'}
# Inspect principal component analysis plot
vsd_PCA_plot <- plotPCA(vsd, intgroup = "Group")
vsd_PCA_plot
```

```{r inpect estimated dispersions, fig.cap='Gráfica de dispersión frente a la media de las cuentas normalizadas.'}
plotDispEsts(ddsMatB)
```
...
La gráfica muestra una variabilidad muy alta en genes con pocas cuentas, que se elimina con la transformación por VST.
...
# Notice this: here we are working with variance transformed data
sampleDists <- dist(t(assay(vsd)))
```

```{r heatmap with pheatmap}
library('pheatmap')
library('RColorBrewer')
...
annotation_columns <- data.frame(Group = colData(ddsMat)[, c("Group")])
rownames(annotation_columns) <- colnames(sampleDistMatrix)

pheatmap(sampleDistMatrix,
         clustering_distance_rows = sampleDists,
         clustering_distance_cols = sampleDists,
         col = colores,
         annotation_row = annotation_columns)
```

Curiosamente, las muestras del grupo experimental ELI se agrupan principalmente en _dos conjuntos diferentes_, mientras que las muestras de los grupos NIT y SFI forman una única agrupación.

### PCA plot
Análisis de componentes principales
```{r PCA plot con datos VST}
plotPCA(vsd, intgroup = "Group")
```

En la gráfica de componentes principales parece que un 70% de la varianza está explicada por el grupo experimental, con las muestras del grupo control (NIT) a una parte de la gráfica, y las muestras de los grupos ELI y SFI entremezcladas en la otra parte.
...
res <- results(ddsMat, contrast = c("Group", "NIT", "ELI"))
res
```

```{r meaning of the columns}
...
Características más estrictas para definir qué grupo de genes se consideran significativos:  
- Disminuir el umbral de la tasa de descubrimiento falsa:
```{r lower false discovery rate threshold}
res.05 <- results(ddsMat, alpha = 0.05)
table(res.05$padj < 0.05)
```

- Aumentar el umbral de log2 fold change:  
```{r raise the log2 fold change threshold}
...
### Cuentas de genes concretos

```{r counts plot}
# Extract the code of the gen with the lowest adjusted p-value
topGene <- rownames(res)[which.min(res$padj)]

library('ggbeeswarm')

# Extract gene counts
geneCounts <- plotCounts(ddsMat, gene = topGene, intgroup = c("Group"), returnData = TRUE)
...
ensembl_codes <- gsub("\\..*", "", row.names(res))
...
Ejemplo del listado de genes, ordenado por p-valor (decreciente), incluyendo el símbolo del gen:
...
idx <- rowMeans(dat) > 1
dat <- dat[idx, ]
mod <- model.matrix( ~ Group, colData(ddsMat))
mod0 <- model.matrix( ~ 1, colData(ddsMat))
svseq <- svaseq(dat, mod, mod0, n.sv = 2)
```

I will try to see if occult batch effect is related with sex:
```{r}
par(mfrow=c(2,1), mar=c(3,5,3,1))
for (i in 1:2) {
  stripchart(svseq$sv[, i] ~ ddsMat$sex, vertical = TRUE, main = paste0("SV", i))
  abline(h=0)
}
```
