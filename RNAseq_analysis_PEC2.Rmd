...
Las listas completas de genes con expresión diferencial entre los diferentes grupos pueden descargarse como archivos en formato CSV XXXXdesde estos enlaces.XXXX
...
Si consideramos aceptable una fracción de falsos positivos del 10%, podemos considerar como significativos todos aquellos genes con un p-valor ajustado por debajo de 0.1, y calcular cuántos genes con expresión diferencial estadísticamente significativa detectamos en cada una de las comparaciones; además los podemos dividir entre genes regulados al alza (log2FC positivo) y regulados a la baja.
...
resSig <- subset(res_SFIvsNIT, padj < 0.1)
...
geneVector_NITvsSFI <- res_SFIvsNIT$entrez[!is.na(res_SFIvsNIT$entrez)]
...
gse_genelist <- res_SFIvsNIT[!is.na(res_SFIvsNIT$padj), ] # Remove NAs
